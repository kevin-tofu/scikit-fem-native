...
public:
    TabulatedAssembler(
        ...
        build_pattern();build_coloring();if(!nonlinear_)build_linear();
    }
...
void native_fem::bind_tabulated_assembler(py::module_&m){py::class_<TabulatedAssembler>(m,"TabulatedAssembler")
    ...
    .def("evaluate",&TabulatedAssembler::evaluate,py::arg("u"),py::arg("external_load")=py::none(),py::arg("with_tangent")=true,py::arg("num_threads")=0)
