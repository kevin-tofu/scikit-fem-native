...
def test_constructor_and_with_elements_match_skfem():
    ...
    direct=skfemntv.Basis(mesh,element,elements=elements)
    restricted=skfemntv.Basis(mesh,element).with_elements(elements)
    ...
    for basis in (direct,restricted):
        ...
        np.testing.assert_allclose(
            skfemntv.asm(form,basis).toarray(),expected.toarray(),
...
def test_restricted_linear_functional_and_interpolation_match_skfem():
    ...
    np.testing.assert_allclose(
        skfemntv.asm(load,basis),skfem.asm(reference_load,reference),
        rtol=3e-13,atol=3e-13,
    )
    np.testing.assert_allclose(
        skfemntv.asm(energy,basis,u=basis.interpolate(coefficients)),
        skfem.asm(
...
def test_cross_basis_and_composite_restriction():
    ...
    trial=skfemntv.Basis(
        mesh,skfemntv.ElementVector(skfemntv.ElementTriP0(),dim=1),
        elements=elements,
    )
    test=skfemntv.Basis(
        mesh,skfemntv.ElementVector(skfemntv.ElementTriP1(),dim=1),
    ...
    np.testing.assert_allclose(
        skfemntv.asm(coupling,trial,test).toarray(),
        skfem.asm(
    ...
    composite=skfemntv.Basis(
        mesh,skfemntv.ElementTriP1()*skfemntv.ElementTriP1(),intorder=4
    ...
    np.testing.assert_allclose(
        skfemntv.asm(mixed,composite).toarray(),
...
def test_element_selection_validation():
    ...
    matrix=skfemntv.asm(mass,empty)
