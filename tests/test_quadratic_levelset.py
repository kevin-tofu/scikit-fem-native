...
def _tet_mesh():
    return skfemntv.MeshTet2.from_mesh(skfemntv.MeshTet())


def _tet_basis(mesh,intorder=2):
    return skfemntv.Basis(
        mesh,skfemntv.ElementVector(skfemntv.ElementTetP2(),dim=1),
        intorder=intorder,
    )


def test_tet10_full_domain_subcells_partition_parent_volume():
    mesh=_tet_mesh()
    quadrature=skfemntv.LevelSet(
        -np.ones(mesh.p.shape[1]),tolerance=0.
    ).cut_quadrature(mesh,intorder=2)

    assert quadrature.diagnostics.total_measure==pytest.approx(1./6.)
    assert len(quadrature.weights)==128
    assert np.all(quadrature.reference_points>=0.)
    assert np.all(quadrature.reference_points.sum(axis=1)<=1.)


def test_tet10_linear_interface_matches_tet4_reconstruction():
    linear=skfemntv.MeshTet()
    quadratic=skfemntv.MeshTet2.from_mesh(linear)
    level_set=skfemntv.LevelSet(
        lambda x:x[0]+x[1]+x[2]-.6,tolerance=0.
    )
...
def test_tet10_cut_basis_reproduces_quadratic_value_and_gradient():
    mesh=_tet_mesh()
    quadrature=skfemntv.LevelSet(
        lambda x:x[0]+x[1]+x[2]-.7,tolerance=0.
    ).cut_quadrature(mesh,intorder=3)
    cut=skfemntv.CutCellBasis(_tet_basis(mesh),quadrature)
    x,y,z=mesh.p
    coefficients=(
        x*x+2.*y*y+3.*z*z+2.*x*y-4.*y*z+x*z
        +3.*x-2.*y+z+1.
    )
    ...
    np.testing.assert_allclose(
        field.value,
        x*x+2.*y*y+3.*z*z+2.*x*y-4.*y*z+x*z+3.*x-2.*y+z+1.,
        atol=4.e-14,
    )
    np.testing.assert_allclose(field.grad[0],2.*x+2.*y+z+3.,atol=5.e-14)
    np.testing.assert_allclose(field.grad[1],4.*y+2.*x-4.*z-2.,atol=5.e-14)
    np.testing.assert_allclose(field.grad[2],6.*z-4.*y+x+1.,atol=5.e-14)


def test_tet10_full_domain_native_assembly_matches_regular_p2_basis():
    mesh=_tet_mesh()
    regular=_tet_basis(mesh,intorder=6)
    cut=skfemntv.CutCellBasis(
        regular,
        skfemntv.LevelSet(-np.ones(mesh.p.shape[1]),tolerance=0.)
        .cut_quadrature(mesh,intorder=6),
    ...
    @skfemntv.BilinearForm
    def mass_stiffness(u,v,w):
        return (1.+w.x[2])*dot(u,v)+ddot(grad(u),grad(v))

    expected=skfemntv.asm(mass_stiffness,regular)
    actual=skfemntv.asm(mass_stiffness,cut,num_threads=2)
    np.testing.assert_allclose(actual.toarray(),expected.toarray(),atol=8.e-14)


def test_tet10_curved_geometry_is_rejected_with_cell_diagnostics():
    mesh=_tet_mesh()
    mesh.p[2,mesh.t[7,0]]+=.01
    level_set=skfemntv.LevelSet(
        lambda x:x[0]+x[1]+x[2]-.5,tolerance=0.
    )

    with pytest.raises(NotImplementedError,match="curved Tet10.*cell 0"):
        level_set.cut_quadrature(mesh)
    with pytest.raises(NotImplementedError,match="curved Tet10.*cell 0"):
        level_set.interface_quadrature(mesh)


def test_tet10_quadratic_level_set_improves_sphere_surface_measure():
    radius=.63
    exact=4.*np.pi*radius**2
    errors=[]
    for points in (5,7):
        axis=np.linspace(-1.,1.,points)
        linear=skfemntv.MeshTet.init_tensor(axis,axis,axis)
        quadratic=skfemntv.MeshTet2.from_mesh(linear)
        level_set=skfemntv.LevelSet(
            lambda x:np.sum(x*x,axis=0)-radius**2,tolerance=0.
        )
        p1=level_set.interface_quadrature(linear,intorder=2)
        p2=level_set.interface_quadrature(quadratic,intorder=2)
        errors.append((
