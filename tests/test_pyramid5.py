...
def basis(*,distorted=False,intorder=4):
    ...
    if distorted:
        ...
        points[:,4]+=(.07,-.04,.08)
        points[:,1]+=(.03,.02,0.)
        mesh=skfemntv.MeshPyramid1(points,mesh.t)
    return skfemntv.Basis(
        mesh,skfemntv.ElementVector(skfemntv.ElementPyramid1()),intorder=intorder
    )
...
def test_pyramid5_constant_strain_patch(distorted):
    ...
    assembler=skfemntv.NativeAssembler.from_basis(
        space,skfemntv.LinearElasticity(young,poisson)
    )
...
def test_pyramid5_material_tangent_and_parallel(material):
    ...
    assembler=skfemntv.MaterialAssembler(space,material)
...
def test_pyramid_mixed_facets_area_normals_and_functional():
    ...
    facets=skfemntv.FacetBasis(
        mesh,skfemntv.ElementVector(skfemntv.ElementPyramid1()),intorder=6
    )
    np.testing.assert_allclose(
