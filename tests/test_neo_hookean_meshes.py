...
def native_space(topology,intorder=4,*,multi=False):
    if topology=="tet10":
        linear=(
            skfn.MeshTet.init_tensor([0.,1.],[0.,1.],[0.,1.])
            if multi else skfn.MeshTet()
        )
        mesh=skfn.MeshTet2.from_mesh(linear)
        element=skfn.ElementTetP2()
    elif topology=="hex27":
        linear=(
            skfn.MeshHex.init_tensor([0.,.5,1.],[0.,1.],[0.,1.])
            if multi else skfn.MeshHex()
        )
        mesh=skfn.MeshHex2.from_mesh(linear)
        element=skfn.ElementHex2()
    elif topology=="wedge6":
        mesh=(
            skfn.MeshWedge1.init_tensor([0.,1.],[0.,1.],[0.,1.])
            if multi else skfn.MeshWedge1()
        )
        ...
        if multi:
            ...
            mesh=skfn.MeshPyramid1(points,cells)
        element=skfn.ElementPyramid1()
    else:
...
def test_neo_hookean_matches_scikit_fem_forms(topology,intorder):
    ...
    actual=skfn.NativeAssembler.from_basis(
        native,skfn.NeoHookean(mu,lmbda)
...
def test_native_basis_rejects_singular_geometry(topology):
    ...
    with pytest.raises(ValueError,match="singular element geometry"):
        skfn.Basis(mesh,skfn.ElementVector(scalar),intorder=2)
